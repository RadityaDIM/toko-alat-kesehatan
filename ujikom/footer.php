<footer class="bg-primary text-white text-center text-lg-start mt-5 ">
  <div class="container p-4">
    <div class="row">
      <!-- Kolom pertama -->
      <div class="col-lg-4 col-md-6 mb-4 mb-md-0 text-center">
        <img
          src="assets/image/icon-healthier.png"
          alt="Logo"
          style="width: 50px; height: 50px; margin: 3px"
          class="" />
        <h5 class="text-uppercase justify-content-center">Healthier</h5>
      </div>

      <!-- Kolom kedua -->
      <div class="col-lg-4 col-md-6 mb-4 mb-md-0 ">
        <h5 class="text-uppercase">Kontak Kami</h5>
        <ul class="list-unstyled mb-0 ">
          <li>
            <i class="fa-solid fa-envelope"></i>healthier@example.com
          </li>
          <li>
            <i class="fa-solid fa-phone"></i> +62 895 422 5297 58
          </li>
          <li>
            <i class="fa-solid fa-location-dot"></i> Jl. Sehat No. 123, Surabaya
          </li>
        </ul>
      </div>

      <!-- Kolom ketiga -->
      <div class="col-lg-4 col-md-6 mb-4 mb-md-0">
        <h5 class="text-uppercase">Ikuti Kami</h5>
        <ul class="list-unstyled d-flex ">
          <li>
            <a href="#" class="text-white mx-2">
              <i class="fa-brands fa-facebook fa-lg"></i>
            </a>
          </li>
          <li>
            <a href="#" class="text-white mx-2">
              <i class="fa-brands fa-twitter fa-lg"></i>
            </a>
          </li>
          <li>
            <a href="#" class="text-white mx-2">
              <i class="fa-brands fa-instagram fa-lg"></i>
            </a>
          </li>
          <li>
            <a href="#" class="text-white mx-2">
              <i class="fa-brands fa-linkedin fa-lg"></i>
            </a>
          </li>
        </ul>
      </div>
    </div>
  </div>

  <div class="text-center p-3 bg-dark">
    © 2024 Healthier | All rights reserved
  </div>
</footer>